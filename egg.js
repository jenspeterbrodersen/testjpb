...
function eggs() {
    ...
    for ( i = 1; i <= Eggcount; i++ ) {
        ...
        // create an array of eggs
        listOfEggs2.push({i, num, min, sec, btime});// better way to organize the data!
        listOfEggs.push(
            { 
                eggnumber : +i,
                weight : +num,
...
console.log("LOG2", listOfEggs2)
    // find max value in array, set timer to the highest value
    ...
    // add 0 to 1 digit numbers (seconds)
        ...
        document.getElementById("egg-list").innerHTML = text += "<div data-eggID=" + byWeight[x].eggnumber + "" +  " data-kogetid=" + byWeight[x].kogemin + sekunder + ">" + "Æg nummer " + byWeight[x].eggnumber + " (" + byWeight[x].weight + " g)" + " skal koge i " + byWeight[x].kogemin + sekunder + " minutter" + "</div>";
    }
...
// create arrays with time to boil in seconds (btime) and ID (eggnumber)
    ...
    if ( timeLeft > 0 ){
        ...
        function match() {
            // $('#egg-list').closest("div").input('[value="2"]').remove();
